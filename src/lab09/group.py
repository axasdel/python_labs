...
class Group:
    ...
    def _ensure_storage_exists(self):  # создание файла с заголовком, если его еще нет
        with open(self.path, "w", newline="", encoding="utf-8") as cf:
            creator = csv.DictWriter(
                cf, fieldnames=["fio", "birthdate", "group", "gpa"]
            )
            creator.writeheader()
    ...
    def add(self, student: Student):  # новый студент в Student
        ...
        ) as cf:  # добавляем нового студента через append
            file_writer = csv.DictWriter(
                cf, fieldnames=["fio", "birthdate", "group", "gpa"]
            )
    ...
    def find(self, substr: str):  # находим студента по фамилии
        data = self._read_all()
        found_student = [
            r for r in data if substr.lower() in r["fio"].lower()
        ]  # независимый от регистра поиск (все в нижнем регистре)
        found_inf = []
